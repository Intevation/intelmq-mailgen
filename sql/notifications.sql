...
CREATE SEQUENCE IF NOT EXISTS intelmq_ticket_seq MINVALUE 10000001;
...
CREATE TABLE IF NOT EXISTS ticket_day (
        initialized_for_day DATE
);
INSERT INTO ticket_day (initialized_for_day)
SELECT '20160101' WHERE NOT EXISTS (SELECT 1 FROM ticket_day);
...
CREATE INDEX IF NOT EXISTS directives_grouping_inserted_at_idx
          ON directives (recipient_address, template_name,
                         notification_format, event_data_format,
                         aggregate_identifier, inserted_at);
CREATE INDEX IF NOT EXISTS directives_events_id_idx
          ON directives (events_id);
CREATE INDEX IF NOT EXISTS directives_sent_id_idx
...
CREATE INDEX IF NOT EXISTS directives_recipient_group_idx
